 ...
 * Includes PKCE state parameter for CSRF protection.
 */
...
export async function exchangeCode(code: string): Promise<void> {
  ...
  // Tapis: client credentials are sent via Basic auth header in some deployments.
  // If a hash is provided in the legacy config, include it. Otherwise omit.
  // The new config does not expose a hash field — Tapis public clients send client_id only.
  if (AUTH_PROVIDER === 'tapis') {
    body.client_id = AUTH_CLIENT_ID;
  }
