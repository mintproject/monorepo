...
describe('buildAddInputVariables', () => {
  it('generates URIs for new inputs when no existingId', () => {
    ...
    expect(vars.inputId).not.toBe(vars.presentationId);
  });
  ...
  it('uses existingId when present (edit mode)', () => {
    ...
    const row = makeInputRow({ existingId: existingDsId, existingPresentationId: existingVpId });
    const vars = buildAddInputVariables('config-1', row);

    expect(vars.inputId).toBe(existingDsId);
    expect(vars.presentationId).toBe(existingVpId);
  });
  ...
  it('maps standard variable and unit FK ids', () => {
    const row = makeInputRow();
    const vars = buildAddInputVariables('config-1', row);

    expect(vars.hasStandardVariable).toBe('https://w3id.org/okn/i/mint/sv-precip');
    expect(vars.usesUnit).toBe('https://w3id.org/okn/i/mint/unit-mm');
  });

  it('passes null for missing standard variable', () => {
    const row = makeInputRow({ standardVariable: null });
    const vars = buildAddInputVariables('config-1', row);
    expect(vars.hasStandardVariable).toBeNull();
  });

  it('passes null for missing unit', () => {
    const row = makeInputRow({ unit: null });
    const vars = buildAddInputVariables('config-1', row);
    expect(vars.usesUnit).toBeNull();
  });

  it('uses variableLabel as presentation label when provided', () => {
    const row = makeInputRow({ variableLabel: 'Precip Variable Label' });
    const vars = buildAddInputVariables('config-1', row);
    expect(vars.presentationLabel).toBe('Precip Variable Label');
  });

  it('falls back to input label for presentation label when variableLabel absent', () => {
    const row = makeInputRow({ variableLabel: undefined });
    const vars = buildAddInputVariables('config-1', row);
    expect(vars.presentationLabel).toBe(row.label);
  });
  ...
  it('maps variable long/short names', () => {
    ...
    const vars = buildAddInputVariables('config-1', row);
    expect(vars.hasLongName).toBe('Daily Precipitation Accumulation');
    expect(vars.hasShortName).toBe('precip_daily');
  });
});
...
describe('buildAddOutputVariables', () => {
  it('generates URIs for new outputs', () => {
