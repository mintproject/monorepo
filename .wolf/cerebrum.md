...
- **ModelCatalogResource lazy-fetch wipes junction columns:** ui/src/screens/models/configure/resources/resource.ts setResources() (line 1419) stores the inline parent payload in `_resources` but then calls `resourceApi.get(id)` per row and stores the entity GET response in `_loadedResources[id]`. Both render (line 479,513) and `getResources()` (line 1642) read from `_loadedResources`, NOT `_resources`. Any field that lives only on a junction (e.g. `isOptional` on modelcatalog_configuration_input, parameter binding values, etc.) and is hoisted only by the parent endpoint's response mapper gets dropped by the lazy refetch. Subclasses with junction fields must capture an inline overlay before super.setResources() and re-merge it into _loadedResources via a requestUpdate() override + sync the overlay on user edits. See ModelCatalogDatasetSpecification._junctionOverlay.
...
- [2026-04-27] Never commit phase/feature work directly on `main` (or `master`). Always create a feature branch (e.g. `gsd/phase-{N}-{slug}`) before the first phase commit, in BOTH the superproject AND any submodule receiving phase commits. Applies to all GSD phases regardless of `branching_strategy: none` in config.
